...
transactions = APIRouter(prefix="/transactions", tags=["Cash Counter"])


def _require_admin(current_user: Customer) -> None:
    if not current_user.is_admin:
        raise NotAdminError("Only admins can move customer balances.")
...
def _with_usernames(
    ...
    names: dict[str, str] = {}
    if ids:
        found = db.query(Customer).filter(Customer.id.in_(ids)).all()
        names = {user.id: user.username for user in found}
    return [
        TransactionListItem(
            id=row.id,
...
def recharge_balance(
    ...
    """Add cash to a customer's balance (admin only)."""
...
def deduct_balance(
    ...
    """Take cash out of a customer's balance; never goes negative (admin only)."""
    _require_admin(current_user)
...
def list_transactions(
    ...
    """Ledger history: admins see everyone, customers see only their own."""
    rows = (
        GetTransactions(db).all()
        if current_user.is_admin
        else GetTransactions(db).for_user(current_user.id)
    )
    return _with_usernames(db, rows)